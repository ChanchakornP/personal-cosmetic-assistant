...
def popularity_score(product: ProductDTO, category_counts: dict[str, int], newest_ts: float, oldest_ts: float) -> float:
    score = 0.0
...
def rank_products(products: List[ProductDTO]) -> List[Tuple[ProductDTO, float]]:
    ...
    scored = [(p, popularity_score(p, category_counts, newest_ts, oldest_ts)) for p in products]
